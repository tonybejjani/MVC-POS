...
class App {
    ...
    _showItemOrderForm(btn) {
     ...
     "data-item-id" attribute of html element */ const { name , price , qty , img  } = menuItems[btn.getAttribute("data-item-id")];
    ...
    _generateMenuCategMarkup() {
        let markup = [];
        for (const { id: categoryId , name , active  } of Object.values(menuCategories))markup.push(`<li class="${active ? "menu--navbar__active" : ""}">
    ...
    _generateMenuItemsMarkup() {
        ...
        for (const { id: categoryId , active  } of Object.values(menuCategories)){
            ...
            for (const [key, { catgId: productCatgId , name , price , qty , img , special_deal  }, ] of Object.entries(menuItems))if (productCatgId === categoryId) markup.push(`<div class="menu--item" data-item-id="${key}">
    ...
    _generateSpecialModal(specialBtnCategId, totalPcs) {
        ...
        for (const [key, { catgId , name , img , special_deal  }] of Object.entries(menuItems))if (catgId === specialBtnCategId && !special_deal) markup.push(`<div class="special-menu--item" data-item-id="${key}">
                        <img src="${img}">
